...
void shrink(gsl_vector_float *x, gsl_vector_float *gamma, float sigma);
float objective(gsl_vector_float *x, float lambda, gsl_vector_float *z);
...
#pragma omp parallel for schedule(static) num_threads(nthreads) shared(X) shared(G) shared(delta) shared(lambda)
  ...
  matvar_t *solution = Mat_VarCreate("X", MAT_C_SINGLE, MAT_T_SINGLE, 2, dims, X->data, MAT_F_DONT_COPY_DATA);
